...
<template>
    ...
    <RouterLink :to="{ name: 'CardDetails', params: { id: username }}">    
        <div class="border border-gray-400 rounded-x1 shadow bg-gray-200 h-full">
        <img :src="faker.image.urlLoremFlickr({category: 'cat'})" class="rounded-t-x1" />
        <div class="p-4">
            <p class="mb-2 text-2x1 font-bold">{{ fullname }}</p>
            <p class="mb-3 font-normal text-gray-700"> {{ bio }}</p>
            <p class="mb-3 font-normal text-gray-700">Username: {{ username }}</p>
        </div>
        </div>
</RouterLink>
</template>